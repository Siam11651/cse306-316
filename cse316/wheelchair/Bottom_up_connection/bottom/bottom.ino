...
bool btCommandGivenFlag;
String command = "";
String state = "";
bool inputBeingGivenFlag;
String tempnum;
bool buzzflag;
bool helpflag;

SoftwareSerial topArduinoCommSerial(2, 3);
SoftwareSerial bluetoothCommSerial(11, 12);
...
void setup() {
  ...
  bluetoothCommSerial.begin(9600);
...
void loop() {
  ...
  for (int i = 0; i < bluetoothCommSerial.available(); i++) {

    char c = bluetoothCommSerial.read();

    Serial.println((char)c);
    inputBeingGivenFlag = true;
    ...
    if (c != '\\') {
      ...
      inputBeingGivenFlag = false;
      break;
    }
  }

  if (state == "x") {
    btCommandGivenFlag = true;
    digitalWrite(BT_MODE_LED, HIGH);
    state = "";

  } else if (state == "y") {
    btCommandGivenFlag = false;
    digitalWrite(BT_MODE_LED, LOW);
    state = "";

  } else if (state == "q") {
    if (btCommandGivenFlag) {
      bluetoothCommSerial.write("1");

    } else {
      bluetoothCommSerial.write("0");
    }
